...
def test_llama_model(device: torch.device, gen_trace: bool, iter: int, debug: bool):
    try:
        batch_size = 1
        seq_length = 32 
        vocab_size = 32000 
        ...
        ti_optimized = my_profile(trace_jit, inputs, iter, f'optim_llama_model_trace_{device}.json', gen_trace, device=device)
        ...
        ti_base = my_profile(model, inputs, iter, f'base_llama_model_trace_{device}.json', gen_trace, device=device)
        ...
        print(f"Error in test_llama_model for device {device}: {str(e)}")
        import traceback
        traceback.print_exc()
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        test_llama_model(torch.device('cuda'), args.gen_trace, args.iter, args.debug)
    test_llama_model(torch.device('cpu'), args.gen_trace, args.iter, args.debug)